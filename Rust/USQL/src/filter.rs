...
use crate::data_structures::{BooleanComparisonOperator, ComparisonOperator, InstructionStack, Operation};
...
fn operate(operation: &Operation, row: &Row) -> Result<bool, RuntimeError> {
    ...
    let mut row_value = match row.get(&operation.column) {
        None => return Err(RuntimeError::InvalidColumn(format!("Invalid Operation: {}", &operation.column).to_string())),
...
fn boolean_operate(a: &bool, b: &bool, boolean_comparison_operator: &BooleanComparisonOperator) -> bool {
...
fn filter_row(instruction_stack: &InstructionStack, row: &Row) -> Result<bool, RuntimeError> {
    ...
    for instruction in instruction_stack {
        match instruction {
            ...
            Instruction::BoolCompOp(bool_operator) => boolean_comparison_operator = bool_operator.clone(),
...
fn filter_table(instruction_stack: InstructionStack, table: Table) -> Result<Table, RuntimeError> {
...
mod tests {
    ...
    fn get_test_table() -> Table {
        Table::from_json_array(get_test_json_table())
    }


    fn get_test_row() -> Row {
        get_test_table().into_iter().collect::<Vec<_>>().get(0).unwrap().clone()
    }
    ...
    fn test_filter_row() {
        ...
        let instruction_stack = compile(tokens).unwrap();
    ...
    fn test_filter_table() {
        let table: Table = get_test_table();

        let input_query = r#"artist = Makaveli"#;
        let tokens = tokenize(input_query).unwrap();
        let instruction_stack = compile(tokens).unwrap();
        let result_table = filter_table(instruction_stack, table.clone()).unwrap();
